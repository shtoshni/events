...
class Controller(BaseController):
    def __init__(self, mlp_size=1024, mlp_depth=1, max_span_width=30, top_span_ratio=0.4, checkpoint=False,
        ...
        for elem_type in IDX_TO_ELEM_TYPE:
            self.mention_mlp[elem_type] = MLP(
                ...
                bias=True, drop_module=self.drop_module, checkpoint=checkpoint)
    ...
    def get_gold_mentions(self, mentions, num_words, flat_cand_mask, ment_type):
        gold_ments = torch.zeros(num_words, self.max_span_width).cuda()
        ment_type = torch.zeros(num_words, self.max_span_width).cuda()
    ...
    def forward(self, example):
        ...
        for ment_idx, ment_type in enumerate(IDX_TO_ELEM_TYPE):
            if ment_type == 'BOTH':
                continue
            mention_logits = torch.squeeze(self.mention_mlp[ment_type](span_embs), dim=-1)
            mention_logits += self.get_mention_width_scores(filt_cand_starts, filt_cand_ends)

            filt_gold_mentions = self.get_gold_mentions(example["mentions"], num_words, flat_cand_mask, ment_idx)
            ...
            if self.training:
                ...
                pred_mention_probs[ment_type] = torch.sigmoid(mention_logits)
                # Calculate Recall
