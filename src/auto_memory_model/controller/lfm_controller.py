...
class LearnedFixedMemController(BaseController):
    def __init__(self, num_cells=10, over_loss_wt=1.0, new_ent_wt=1.0, **kwargs):
        ...
        self.memory_net = LearnedFixedMemory(
            num_cells=num_cells, hsize=self.ment_emb_to_size_factor[self.ment_emb] * self.hsize + 2 * self.emb_size,
        ...
        self.coref_loss_wts = torch.tensor([1.0] * self.num_cells + [self.new_ent_wt]).cuda()

        self.over_loss_wts = torch.tensor([1.0] * (self.num_cells + 2)).cuda()
